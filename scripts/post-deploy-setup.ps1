...
# Configuración ÚNICA del DynamoDB Stream trigger
# Ejecutar DESPUÉS de que `npx ampx sandbox` termine de desplegar

Write-Host ""
Write-Host "═══════════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host "  🎤 AWS PUEBLA - CONFIGURACIÓN STREAM TRIGGER" -ForegroundColor Cyan
Write-Host "═══════════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host ""
Write-Host "✅ La infraestructura (IaC) ya está configurada automáticamente" -ForegroundColor Green
Write-Host "⚠️  Solo necesitas conectar el Stream trigger (una sola vez)" -ForegroundColor Yellow
Write-Host ""
...
if (-not $tableName) {
    Write-Host "❌ No se encontró la tabla SpeakerApplication" -ForegroundColor Red
...
if ($streamEnabled -eq "True") {
    Write-Host "✅ Streams ya están habilitados" -ForegroundColor Green
} else {
    Write-Host "⚠️  Habilitando streams..." -ForegroundColor Yellow
...
$lambdaArn = aws lambda list-functions --query "Functions[?contains(FunctionName, 'process-speaker-application')].FunctionArn" --output text

if (-not $lambdaArn) {
    Write-Host "❌ No se encontró la Lambda" -ForegroundColor Red
    exit 1
}

Write-Host "✅ Lambda encontrada: $lambdaArn" -ForegroundColor Green
...
if ($existingMapping) {
    ...
    Write-Host "⚙️  Creando Event Source Mapping..." -ForegroundColor Yellow
...
Write-Host ""
Write-Host "🎯 Siguiente paso: Configurar SES para envío de emails" -ForegroundColor Yellow
