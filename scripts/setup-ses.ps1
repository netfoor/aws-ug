...
# Verifica un email para testing o muestra cómo salir del sandbox

Write-Host "📧 Configuración de Amazon SES" -ForegroundColor Cyan
Write-Host ""
...
if ($account.ProductionAccessEnabled) {
    Write-Host "✅ Cuenta SES en modo PRODUCCIÓN" -ForegroundColor Green
    Write-Host "   Puedes enviar emails a cualquier dirección" -ForegroundColor Gray
} else {
    Write-Host "⚠️  Cuenta SES en modo SANDBOX" -ForegroundColor Yellow
...
if ($identities.EmailIdentities.Count -eq 0) {
    Write-Host "   ⚠️  No hay identidades verificadas" -ForegroundColor Yellow
} else {
...
# Opción 1: Verificar email individual
Write-Host "OPCIÓN 1: Verificar un email para testing" -ForegroundColor Yellow
Write-Host "────────────────────────────────────────" -ForegroundColor Gray
$email = Read-Host "Ingresa el email a verificar (Enter para omitir)"

if ($email) {
    try {
        Write-Host "📤 Enviando email de verificación a: $email" -ForegroundColor Yellow
        aws sesv2 create-email-identity --email-identity $email
        
        Write-Host "✅ Email de verificación enviado!" -ForegroundColor Green
        Write-Host "📬 Revisa tu inbox y haz click en el link de verificación" -ForegroundColor Cyan
        Write-Host ""
        Write-Host "⏳ Esperando verificación (presiona Enter cuando lo hayas verificado)..." -ForegroundColor Yellow
        Read-Host
        ...
        if ($identityDetails.VerifiedForSendingStatus) {
            ...
            Write-Host "⚠️  Email aún no verificado. Revisa tu inbox." -ForegroundColor Yellow
        }
    } catch {
...
Write-Host ""
Write-Host "OPCIÓN 2: Usar AWS SES Simulator (para testing)" -ForegroundColor Yellow
Write-Host "────────────────────────────────────────────────" -ForegroundColor Gray
Write-Host "Puedes usar estos emails de prueba (no requieren verificación):" -ForegroundColor Gray
Write-Host "   - success@simulator.amazonses.com (simula éxito)" -ForegroundColor Green
...
Write-Host "   Actualizar SENDER_EMAIL en backend.ts con un email verificado" -ForegroundColor Gray
Write-Host "   Ejemplo: 'noreply@awspuebla.com' o tu email verificado" -ForegroundColor Gray
